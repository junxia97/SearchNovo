...
from novobench.models.searchnovo.layers.decoder import PeptideSearchDecoder
    ...
    **kwargs : Dict
    ...
    def __init__(
        ...
        **kwargs: Dict,
        ...
        self.decoder = PeptideSearchDecoder(
            dim_model=dim_model,
            n_head=n_head,
            dim_feedforward=dim_feedforward,
            n_layers=n_layers,
        ...
        self.celoss2 = torch.nn.CrossEntropyLoss(
            ignore_index=0, label_smoothing=train_label_smoothing, reduction = "none"
        )
    ...
    def forward(
        self, spectra: torch.Tensor, precursors: torch.Tensor, peptide_ref: List[str], ref_similarity: torch.Tensor
        ...
        return self.beam_search_decode(
            ...
            peptide_ref,
            ref_similarity.to(self.decoder.device)
        )
    ...
    def beam_search_decode(
        ...
        pred, _ = self.decoder(None, precursors, memories, mem_masks, peptide_ref, ref_similarity)
        ...
        #NOTE new for searchnovo
        similarity_scores = einops.repeat(ref_similarity, "B 1 -> (B S) 1", S=beam)
        ref_tokens = self.decoder.prepare_tokens(peptide_ref)
        ref_tokens = einops.repeat(ref_tokens, "B L -> (B S) L", S=beam)
        ...
        for step in range(0, self.max_length):
            ...
            scores[~finished_beams, : step + 2, :], _ = self.decoder(
                ...
                ref_tokens[~finished_beams, :],
                similarity_scores[~finished_beams, :]
            )
    ...
    def _forward_step(
        ...
        precursors: torch.Tensor,
        sequences: List[str],
        peptide_ref: List[str], 
        ref_similarity : torch.Tensor,
        mode = "default"
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        """
        if mode == "null":
            empty = [""] * len(peptide_ref) 
            return self.decoder(sequences, precursors, *self.encoder(spectra), empty, ref_similarity)
        return self.decoder(sequences, precursors, *self.encoder(spectra), peptide_ref, ref_similarity)

    def training_step(
        self,
        batch: Tuple[torch.Tensor, torch.Tensor, List[str], List[str], torch.Tensor],
        *args,
        mode: str = "train",
    ) -> torch.Tensor:
        """
        ...
        if mode == "train":
            pred_null, truth = self._forward_step(*batch, mode = "null")
            pred_null = pred_null[:, :-1, :].reshape(-1, self.decoder.vocab_size + 1)
            loss1 = self.celoss(pred, truth.flatten())
            loss2 = self.celoss(pred_null, truth.flatten())
    ...
    def validation_step(
        self, batch: Tuple[torch.Tensor, torch.Tensor, List[str], List[str], torch.Tensor], *args
    ) -> torch.Tensor:
        """
        ...
        if not self.saved_path == "":
            ...
            for spectrum_preds in self.forward(batch[0], batch[1], batch[3], batch[4]): #Note changed
                if spectrum_preds == []:
