...
class SearchnovoDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, float, int, torch.Tensor]:
        ...
        similarity = self.df[idx, "similarity_score"]
        ...
        peptide_ref = ''
        if 'ref_sequence' in self.df.columns:
            peptide_ref = self.df[idx, 'ref_sequence'] 


        spectrum = torch.stack([mz_array, intensity_array], dim=1)
        
        return spectrum, precursor_mz, precursor_charge, peptide, peptide_ref, similarity


class SearchnovoDataModule:
    """
    ...
    def __init__(
        ...
        self.dataset = SearchnovoDataset(df)
...
def collate_batch(batch: List[Tuple[torch.Tensor, float, int, str]]) -> Tuple[torch.Tensor, torch.Tensor, np.ndarray]:
    ...
    """
    spectra, precursor_mzs, precursor_charges, spectrum_ids, peptide_ref, similarity = zip(*batch)
    ...
    peptide_ref = np.array(peptide_ref)
    similarity = torch.tensor(similarity).unsqueeze(-1)

    return spectra, precursors, spectrum_ids, peptide_ref, similarity
